...
| **Message queue** | Send while a chat is answering and the message waits its turn: when the answer ends, the next queued message starts by itself (in this window or not, and after a restart). Drag queued messages to reorder, click to edit, **now** puts one into the running answer straight away (⌘Enter does that as you send). Stop pauses the queue. Any number of chats answer at once on hosted models; chats on the local model take turns, since they share one model server (`max_parallel` in settings.json sets a cap if you want one). |
| **Other agents' sessions** | Conversations from the Claude CLI and Claude's desktop app, Codex and OpenCode, each in its own sidebar section. Open one and keep going: Orbit continues it in that agent's own session. |
| **Chats on another machine** | Claude Code chats that run over SSH on a cluster login node or server — files edited and commands run there, models from your Mac through a token-protected tunnel, nothing left running afterwards. |
